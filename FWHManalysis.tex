...
The first is a damped random walk model~\citep{zeljkoBook},
\begin{equation}
...
we obtain the $\tau$ distribution vs. the number of fields in each
...
There are a total of 12 runs with the number of fields larger than
600.
...
Combining fit results for all camera columns and optical bands for run 4874
gives $\beta = -1.14\pm 0.05$.
Making the same fits for all the 108 runs in Stripe 82, 
we obtained the $\beta$ distribution vs. the number of fields in each
run, as shown in Fig.~\ref{fig:hist} (right).
The shorter runs give $\beta$ values with a larger variance.
Most of the longer runs have power-law index roughly between $-1.4$ and $-0.9$.
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{FIGURES/taubeta.png}
\caption{Left: The damped random walk characteristic
  timescale $\tau$ for all 108 runs in Stripe 82 vs. the number of
  fields in each run. Right: The power-law index $\beta$ for all 108 runs vs. the number of
  fields in each run.
\label{fig:hist}}
\end{figure}
...
%note: fits do not look as good as Fig 4 in Racine 1997.
