...
(dashed) lines are also shown. For the ensemble behavior of best-fit $\alpha$, see Figure~\ref{fig:alpha_fwhm}. 
...
Figure~\ref{fig:fwhm_lambda} shows such fits for run 4874. Significant deviation 
...
the results). Figure~\ref{fig:alpha_fwhm} shows a scatter plot of the resulting
...
Figure~\ref{fig:alpha_fwhm}. Similar correlations have been seen in Subaru images 
...
Results for the $r$-band are shown in Figure~\ref{fig:spatial}.
...
For comparison, Figure ~\ref{fig:spatial} also shows results from the CFHT PSF 
...
The three curves in Figure~\ref{fig:spatial} appear to be quantitatively
...
Figure~\ref{fig:psd} shows the temporal power spectral density (PSD) of the
...
seen in column 6, it is clear from Figure~\ref{fig:psd} that the temporal behavior
...
The solid curves in Figure~\ref{fig:psd} show fits using this model,
...
run, as shown in Figure~\ref{fig:hist} (left).
...
Best-fits are illustrated for run 4874 are in Figure~\ref{fig:psd} (dashed lines).
...
$\beta$ distribution vs. the duration of each run shown in Figure~\ref{fig:hist} (right).
...
Figure~\ref{fig:fdt} (left) shows one example of such fits. This functional form is 
somewhat inspired by damped random walk model, where $\gamma=1$ and the 
term in brackets is raised to the power of a half. 
For this particular fit in Figure~\ref{fig:fdt} (left), $\gamma$ is very close to 1.
%For illustration, Figure~\ref{fig:fdt} 
...
the duration of each run is shown in Figure~\ref{fig:fdt} (right) (somewhat 
arbitrarily,
